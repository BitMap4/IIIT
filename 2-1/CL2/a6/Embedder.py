...
class CooccurrenceMatrix:
    ...
    def create_matrix(self, sentences: List[str], window_size: int) -> np.ndarray:
        """Create co-occurrence matrix using dense numpy array implementation."""
        ...
        tokenized_sentences = [self.preprocessor._clean_text(s) for s in sentences]
        
        for tokens in tqdm(tokenized_sentences, desc="Processing co-occurrences"):
            token_indices = [self.preprocessor.word_to_idx[token] 
                           for token in tokens 
                           if token in self.preprocessor.word_to_idx]
    ...
    def get_matrix(self) -> str:
        """Print the full co-occurrence matrix with word labels."""
        if self.matrix is None:
            raise ValueError("Matrix not created yet. Run create_matrix() first.")
...
class WordEmbeddings:
    def __init__(self, config: AnalysisConfig) -> None:
        ...
        self.S: Optional[NDArray[np.float32]] = None
    
    def create_embeddings(self, matrix: np.ndarray) -> NDArray[np.float32]:
        ...
        # Take only the top k components
        ...
        self.S = S[idx].astype(np.float32)
